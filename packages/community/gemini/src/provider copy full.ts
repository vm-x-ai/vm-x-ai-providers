...
import type { Part, SingleRequestOptions, GenerationConfig, GenerativeModel } from '@google/generative-ai';
...
interface ChatParamsNonStreaming {
  request: string | Array<string | Part>;
  requestOptions?: SingleRequestOptions;
}

interface ChatParamsStreaming extends ChatParamsNonStreaming {
  stream: boolean;
}

export type CallAPIRequest = ChatParamsNonStreaming | ChatParamsStreaming;
...
export class GeminiLLMProvider extends BaseCompletionProvider<GoogleGenerativeAI> implements ICompletionProvider {
  ...
  protected override async createClient(
    ...
    const genAI = new GoogleGenerativeAI(connection.config.apiKey);
  ...
  private async callCompletion(
    ...
    const client = await this.createClient(connection, model);

    const generationConfig: GenerationConfig = {
      ...(request.config?.temperature !== undefined && { temperature: request.config.temperature }),
      ...(request.config?.top_p !== undefined && { topP: request.config.top_p }), // top_p -> topP
      ...(request.config?.top_k !== undefined && { topK: request.config.top_k }), // top_k -> topK
      ...(request.config?.max_tokens !== undefined && { maxOutputTokens: request.config.max_tokens }), // max_tokens -> maxOutputTokens
      ...(request.config?.response_format && { responseMimeType: request.config.response_format }), // response_format -> responseMimeType
      ...(request.config?.response_schema && { responseSchema: request.config.response_schema }), // response_schema -> responseSchema
      ...(request.config?.presence_penalty !== undefined && { presencePenalty: request.config.presence_penalty }), // presence_penalty -> presencePenalty
      ...(request.config?.frequency_penalty !== undefined && { frequencyPenalty: request.config.frequency_penalty }), // frequency_penalty -> frequencyPenalty
      ...(request.config?.responseLogprobs !== undefined && { responseLogprobs: request.config.responseLogprobs }),
      ...(request.config?.logprobs !== undefined && { logprobs: request.config.logprobs }),
    };

    const chatSession = client.startChat({
      history: request.messages.map<MessageParam>((msg) => ({
        role: msg.role,
    ...
    let toolChoice: FunctionCallingMode = FunctionCallingMode.AUTO;
    ...
    const geminiRequest: CallAPIRequest = {
      ...(request.config || {}),
      ...
      messages: this.parseRequestMessagesToGeminiFormat(request),
    };

    this.logger.log({ request: geminiRequest }, 'Calling Anthropic API');
  ...
  private parseRequestMessagesToGeminiFormat(request: CompletionRequest) {
  ...
  private async parseStreamingResponse(
    request: CompletionRequest,
    data: Stream<GoogleGenerativeAI.RawMessageStreamEvent>,
    model: ResourceModelConfig,
    metadata: CompletionMetadata,
    startTime: number,
    observable: Subject<CompletionResponse>,
  ): Promise<{ message: GoogleGenerativeAI.Message; timeToFirstToken: number }> {
    let timeToFirstToken = 0;
    const contentBlocks: ContentBlock[] = [];

    let message: GoogleGenerativeAI.Message = {
      id: '',
      model: '',
      role: 'assistant',
      content: [],
